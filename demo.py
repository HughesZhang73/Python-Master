...
def draw_arrow(point, color='Grey', fill='black'):
    ...
    canvas.create_rectangle(col_bir - np.ceil(0.2 * axis), row_bir - np.ceil(0.2 * axis), col_bir + np.ceil(0.2 * axis),
                            row_bir + np.ceil(0.2 * axis), fill='black')
    
    # draw arrow
    canvas.create_line(col_bir, row_bir, col_end, row_end, width='2', fill=color)
    canvas.create_oval(int(col_bir - axis / 15), int(row_bir - axis / 15), int(col_bir + axis / 15),
                       int(row_bir + axis / 15), outline=color, fill=fill)
...
def draw_unit(point, color, flag=0):
    ...
    # show info
    point.f_set()
    t_f = tk.Label(window, text='f=%d' % point.f, bg='black', font=('Arial', np.max([1, np.int(np.floor(100 / n))])),
                   foreground='green', width=3, height=1)
    t_g = tk.Label(window, text='g=%d' % point.g, bg='black', font=('Arial', np.max([1, np.int(np.floor(100 / n))])),
                   foreground='green', width=3, height=1)
    t_h = tk.Label(window, text='h=%d' % point.h, bg='black', font=('Arial', np.max([1, np.int(np.floor(100 / n))])),
                   foreground='green', width=3, height=1)
...
def l_callback(event):
    ...
    if start != None:
        canvas.create_rectangle(cs[start[1]] + 1, rs[start[0]] + 1, cs[start[1] + 1] - 1, rs[start[0] + 1] - 1,
                                fill='black')
...
def hit1():
    ...
    if do == 1:
        ...
        n = int(text.get())
        tk.messagebox.showinfo(title='tips',
                               message='Use left mouse button to select start node;' + '\n' + 'Use middle mouse button to select obstructions;'
